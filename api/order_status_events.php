...
// Function to emit an event to a log file that the SSE endpoint can read
function emitOrderEvent($orderId, $status) {
    $eventData = [
        'order_id' => $orderId,
        'status' => $status,
        'timestamp' => time()
    ...
    try {
        ...
        $query = "INSERT INTO order_events (order_id, event_type, event_data) VALUES (?, 'status_change', ?)";
        $stmt = $mysqli->prepare($query);
        $eventJson = json_encode($eventData);
        $stmt->bind_param('is', $orderId, $eventJson);
        $stmt->execute();
    } catch (Exception $e) {
...
$event = emitOrderEvent($orderId, $status);
